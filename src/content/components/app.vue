		...
		<div v-if="visible" class="box-card-parent">
			...
			>
				<template #header>
					...
					<el-button style="margin-left: 1rem" type="primary" @click="getStaticMonitor">获取性能数据</el-button>
<!--					<el-button style="margin-left: 1rem" type="primary" @click="startDynamicMonitor">动态性能数据</el-button>-->
				...
				>
					...
					<el-table-column fixed prop="initiatorType" sortable resizable label="标签类型" width="auto" />
					<el-table-column fixed prop="lookupTime" sortable resizable label="解析时间ms" width="auto" />
					<el-table-column fixed prop="tcpTime" sortable resizable label="tcp耗时ms" width="auto" />
					<el-table-column fixed prop="sslTime" sortable resizable label="ssl耗时ms" width="auto" />
					...
					<el-table-column fixed prop="startTime" sortable resizable label="开始时间ms" width="auto" />
...
// 获取iframe下所有资源类型的性能条目
			...
			for (let i = 0; i < entries.length; i++) {
				...
				this.tableData.push({
					...
					startTime: parseFloat(entries[i].startTime.toFixed(2)),
					// responseEnd: parseFloat(entries[i].responseEnd.toFixed(2)),
					duration: parseFloat(entries[i].duration.toFixed(2))
				})
			}
		},
		closeCard(){
		...
		startDynamicMonitor() {
			this.tableData = []
			this.observer = new PerformanceObserver((entries,obj) => {
				console.log(obj)
				for (let i = 0; i < entries.length; i++) {
					console.log(entries[i])
					this.tableData.push({
						...
						initiatorType: entries[i].initiatorType,
						fileType: entries[i].name.replace(/(^.*)(\..*$)/, '$2'),
						lookupTime: (entries[i].domainLookupEnd - entries[i].domainLookupStart).toFixed(2),
						connectionTime: (entries[i].requestStart - entries[i].secureConnectionStart).toFixed(2),
						responseTime: (entries[i].responseEnd - entries[i].responseStart).toFixed(2),
						transferSize: (entries[i].transferSize / 1000).toFixed(2),
						startTime: entries[i].startTime.toFixed(2),
						responseEnd: entries[i].responseEnd.toFixed(2),
						duration: entries[i].duration.toFixed(2)
					})
				}
			});
			this.observer.observe({
				entryTypes: [this.selectedType],
			});
		},
		stopDynamicMonitor() {
